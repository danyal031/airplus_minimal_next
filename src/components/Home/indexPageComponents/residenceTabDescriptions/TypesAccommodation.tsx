...
const TypesAccommodation = () => {
  ...
  const renderTypesOfResidencesOnDesktop = () => {
    return (
      <div className="w-3/4 hidden md:flex items-center justify-center gap-3 justify-self-center">
        {residences.map((item) => (
          <div
            key={item.id}
  ...
  // for mobile
  const renderTypesOfResidencesOnMobile = () => {
    return (
      <div className="md:hidden grid grid-cols-2 items-center gap-3 justify-self-center">
        {residences.map((item) => (
          <div
            key={item.id}
            className="flex items-center justify-center gap-3 bg-paper p-5 min-w-40 max-w-40 rounded-xl border-2 hover:border-primary-main hover:cursor-pointer"
  ...
  return (
    <div className="grid grid-cols-1 gap-4">
      ...
      {renderTypesOfResidencesOnDesktop()}
      {renderTypesOfResidencesOnMobile()}
    </div>
  );
};
